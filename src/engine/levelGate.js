...
// consumed. The level is always the account's real level (User#getLevel = levelForXp(xp)), never the
// public level. Gear that is already equipped is never unequipped by these gates.
...
function levelOfUserDoc(userDoc) {
	return levelForXp(userDoc?.xp || 0);
}
